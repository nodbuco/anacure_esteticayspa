...
9. **Que el fondo nunca quede vacío.** `escena` es un objeto de módulo y sobrevive a la navegación sin recarga, así que `ScrollScenes` lo reinicia al montarse y al desmontarse. `Flores` no se salta ningún fotograma hasta haber colocado las flores al menos una vez, y las mallas nuevas nacen con sus instancias ocultas (escala 0). Antes, quien salía del inicio por un enlace del pie y volvía encontraba la escena «apagada»: las flores no se colocaban y se veía un único pétalo en el centro. Si el navegador libera la GPU y no la devuelve en un segundo, `FlowerField` vuelve al fondo 2D.
...
- El retrato no se anima por tiempo sino cuando la foto está lista, para que nunca se vea a medio cargar ni recortada:
  ...
  2. Cuando la imagen está descargada y decodificada y el arco está en pantalla, la figura recibe `data-revelado`: el velo borroso se disuelve sobre la foto nítida mientras las dos se asientan (escala 1,07 → 1, como un enfoque), la línea del arco se dibuja de abajo arriba, aparece el pie y, al final, el sello «+10 años cuidando tu piel» (cristal que en escritorio asoma por el borde izquierdo del arco).
