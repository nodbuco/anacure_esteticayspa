 ...
 * La columna de texto es lo bastante ancha para que «CONVIERTE EN» quepa en un renglón: el
 * titular ocupa cuatro líneas, no cinco, y la nota de la valoración entra en la primera pantalla.
 */
export function Hero() {
  return (
    ...
    >
      <Container className="grid w-full items-center gap-12 py-12 sm:py-14 lg:grid-cols-[1.18fr_0.82fr] lg:gap-10 lg:pb-[calc(6rem-var(--alto-promo,0rem))] lg:pt-6 pantalla-baja:pt-3">
        <div className="max-w-2xl">
          ...
          <h1 id="hero-titulo" className="entrada-subir titular mt-6 text-display-xl text-tinta pantalla-baja:mt-3 pantalla-baja:text-[length:min(var(--text-display-xl),9svh)]">
            Donde el cuidado se convierte en <span className="text-purpura">experiencia</span>
          </h1>
          <p className="entrada-aparecer prosa mt-7 text-lead text-gris pantalla-baja:mt-4" style={retraso(0.25)}>
            Medicina estética de última generación y un spa hecho para volver a ti, en El Banco y en Aguachica.
          </p>
          <div className="mt-9 flex flex-col gap-3 sm:flex-row sm:items-center pantalla-baja:mt-6">
          ...
          <p className="entrada-aparecer mt-6 inline-flex items-center gap-2 text-sm text-tinta-suave pantalla-baja:mt-3" style={retraso(0.65)}>
      ...
      {/* Indicación de scroll (solo escritorio y con altura de sobra: en portátiles bajos chocaría con los botones) */}
      <div aria-hidden="true" className="entrada-aparecer absolute bottom-6 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-2 lg:[@media(min-height:46rem)]:flex" style={retraso(1.4)}>
